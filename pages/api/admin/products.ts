...
const updateProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  if ( images.length < 2 ) {
    return res.status(400).json({message: `2 images min are required`})
  }
  ...
  try {
    ...
    if ( !product ) {
      await db.disconnect()
      return res.status(400).json({message: `Product ID do not exist`})
    }
...
const createProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  if ( images.length < 2 ) {
    return res.status(400).json({
      message: 'Min 2 images are required'
    })
  }
  ...
  try {
    ...
    if ( productInDB ) {
      await db.disconnect()
      return res.status(400).json({
        message: 'Already exist a product by that slug'
      })
    }
